...
class DataHelper:
    def __init__(self, gz=True, config=None):
        ...
        self.gz = gz  # true; used to determine the preprocessed data type
    ...
    def get_feature_file(self, tag: str) -> str:
        """
        Get the file path for the data.
        Args:
            tag (str): in our case it's either "train" or "dev"
        Return:
            (str): file path
        """
        # todo: assert could be added to avoid wrong tag input
    ...
    @property
    def dev_loader(self):
        return self.DataIterator(*self.load_dev(),   # feature, example_dict -> {id: example}
                                 bsz=self.config.eval_batch_size,
                                 device='cuda:{}'.format(self.config.model_gpu),
                                 sent_limit=self.sent_limit,   # 100 by default
                                 entity_limit=self.entity_limit,  # 80 by default
                                 sequential=True,
                                )

    @property
    def train_loader(self):
        return self.DataIterator(*self.load_train(),   # feature, example_dict
