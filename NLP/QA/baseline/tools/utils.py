...
def convert_to_tokens(example, features, ids, y1, y2, q_type):
    # example and features are the original data
    # ids: article/document ids in the batch
    # y1: predicted start position of answer span
    # y2: predicted end position of answer span
    # q_type: predicted answer type
    answer_dict = dict()
    
    for i, qid in enumerate(ids):   # article id
        answer_text = ''
        if q_type[i] == 0:
            # if answer is text context from document
